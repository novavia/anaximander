...
class NxMeta(type):
    ...
    __basename__ = 'Nx'
    __archetype__ = None
    __metacharacters__ = OrderedDict()
...
def nxmeta(basetype, basename=None):
    ...
    metacharacters = OrderedDict(getattr(basemeta, '__metacharacters__', {}))
...
class ArcheType(metaclass=ArchMeta):
    ...
    def __new__(mcl, name, bases, namespace, **kwargs):
        ...
        if mcl.__archetype__ is None:
            ...
            # Adds ObjectTypeProperties to basetype for metacharacters
            base_metacharacters = getattr(basetype, '__metacharacters__', {})
            for k, v in mcl.__metacharacters__.items():
                # Set the ObjectTypeProperty on the basetype
                typeproperty = getattr(mcl, k)
                # This checks that the metacharacter isn't a carryover
                # from a parent archetype, in which case this step is skipped.
                if not base_metacharacters.get(k, None) == v:
                    typeproperty.set_typical_property(basetype)
                setattr(archetype, typeproperty.cache, None)
            return archetype
        else:
